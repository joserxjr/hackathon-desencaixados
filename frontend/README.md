...
- **Ajustes de Leitura e Visualização** (Injetados via *Content Scripts* dinamicamente):
  - **Tamanho da Fonte**: Escalonamento de todo o texto da página (`rem`, `px`, etc).
  - **Espaçamento de Letras**: Ajuste no *letter-spacing* para melhorar o rastreio visual.
...
- **Web Extensions API (Manifest V3)**: Arquitetura baseada em Chrome APIs para máxima segurança e compatibilidade cross-browser.
  - *Service Workers* (`background.js`) para processamento silencioso.
  - *Content Scripts* (`content.js`) para manipulação e injeção do DOM.
...
### Mozilla Firefox
1. Abra o navegador e digite na barra de endereços: `about:debugging#/runtime`
